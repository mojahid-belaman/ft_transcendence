...
export class Ball {
  ...
  public detect_Collision(player_One: Player): boolean {
    ...
    let paddle_bottom: number =
      player_One.getPaddle().get_PaddleY() + GameVariable._paddle_Height;
    let paddle_left: number = player_One.getPaddle().get_PaddleX();
    let paddle_right: number =
      player_One.getPaddle().get_PaddleX() + GameVariable._paddle_Width;
  ...
  public direction_Ball(player_One: Player): void {
    if (this.detect_Collision(player_One)) {
      ...
      this._collidePoint =
        this._collidePoint / (GameVariable._paddle_Height / 2);

      let angleRad: number = (Math.PI / 4) * this._collidePoint;

      let direction: number =
        this._ball_X < GameVariable._canvas_Width / 2 ? 1 : -1;
